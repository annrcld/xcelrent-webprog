...
try {
    ...
    $uploadDir = __DIR__ . '/../uploads/cars/'; // Changed to public uploads instead of admin
    if (!is_dir($uploadDir)) {
    ...
    for ($i = 0; $i < count($_FILES['photos']['name']); $i++) {
        ...
        if (isset($_FILES['photos']['error'][$i]) && $_FILES['photos']['error'][$i] === UPLOAD_ERR_NO_FILE) {
            continue;
        }
        ...
        $fileSize = $_FILES['photos']['size'][$i] ?? 0;
        $fileType = $_FILES['photos']['type'][$i] ?? '';

        if (empty($fileTmpPath) || empty($fileName)) {
            $errors[] = "File " . ($i + 1) . " is not properly uploaded.";
            continue;
        }
    ...
    if (count($uploadedPhotos) > 0) {
        ...
        $updateStmt = $conn->prepare("UPDATE cars SET image = ? WHERE id = ?"); // Changed from car_image to image
    ...
    error_log("Error in upload_car_photos.php: " . $e->getMessage());
